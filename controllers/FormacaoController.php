...
class FormacaoController extends FormacaoModel
{
    ...
    {
        if ($excel != 0 && $ano != 0):
            $sql = "SELECT p.numero_processo, fc.protocolo, pf.id, pf.nome, pro.programa, c.cargo AS 'funcao', l.linguagem, pf.email, s.status
                                                      FROM pedidos AS p
            ...
            return DbModel::consultaSimples("SELECT p.id, p.origem_id, p.valor_total, p.data_kit_pagamento, p.numero_processo, p.numero_parcelas, p.pessoa_fisica_id, p.valor_total, p.numero_processo_mae, 
                                                            p.forma_pagamento, p.justificativa, p.observacao, p.verba_id, s.status, fc.protocolo, pf.nome, c.cargo 
                                                  FROM pedidos AS p
